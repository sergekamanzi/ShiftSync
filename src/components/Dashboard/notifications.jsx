...
function NotificationCard({ item }) {
	if (item.kind === 'urgent') {
		return (
			<article className="overflow-hidden rounded-[26px] border border-rose-200/80 bg-white shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
					...
					<div className="flex items-start gap-4">
						...
						<div className="min-w-0">
							...
							<div className="mt-4 flex flex-wrap gap-3">
								<button className="rounded-xl bg-[#d92d20] px-4 py-2.5 text-sm font-extrabold text-white shadow-[0_12px_24px_rgba(217,45,32,0.24)]">{item.action}</button>
			...
			<article className="overflow-hidden rounded-[26px] border border-slate-200/80 bg-white shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
				<div className="p-5 sm:p-6">
					...
					<div className="mt-4 flex flex-wrap items-center justify-between gap-3">
						...
						<button className="rounded-xl bg-[#0f51ff] px-4 py-2.5 text-sm font-extrabold text-white shadow-[0_12px_24px_rgba(15,81,255,0.26)]">{item.action}</button>
					</div>
				</div>
			</article>
		)
	...
	return (
		<article className="rounded-[26px] border border-slate-200/80 bg-[#eef3ff] p-5 shadow-[0_12px_30px_rgba(15,23,42,0.06)] sm:p-6">
			<div className="flex items-start justify-between gap-4">
				<div className="flex items-start gap-4">
					<div className="flex h-12 w-12 items-center justify-center rounded-full bg-white text-slate-500 shadow-[0_8px_20px_rgba(15,23,42,0.06)]">
			...
			<div className="flex h-screen w-full overflow-hidden border border-white/80 bg-white/85 shadow-[0_30px_90px_rgba(15,23,42,0.14)] backdrop-blur-xl">
				<aside className="flex w-full shrink-0 flex-col overflow-hidden border-r border-slate-200/80 bg-[#f2f6ff]/80 px-5 py-6 xl:fixed xl:left-0 xl:top-0 xl:h-screen" style={{ width: '290px', maxWidth: '290px' }}>
					...
					<nav className="space-y-2 text-[14px] font-medium text-slate-600">
						...
						<Link className="flex items-center gap-3 rounded-xl bg-white px-4 py-3 font-semibold text-[#0f51ff] shadow-[0_10px_24px_rgba(15,23,42,0.06)]" to="/notifications"><FiBell className="h-4 w-4" /> Notifications</Link>
					...
					<div className="mt-8 space-y-3">
						<button className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#0f51ff] px-4 py-3 text-sm font-bold text-white shadow-[0_14px_32px_rgba(15,81,255,0.32)] transition hover:-translate-y-0.5 hover:bg-[#0b44de]"><FiPlus className="h-4 w-4" /> Create Notification Rule</button>
					...
					<header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/75 px-4 py-4 backdrop-blur-xl sm:px-6 xl:px-8">
						...
						<div className="mt-4 flex flex-col gap-4 xl:mt-0 xl:flex-row xl:items-center xl:justify-between">
							<label className="relative w-full max-w-3xl">
								...
								<input type="search" placeholder="Search notifications, employees, or policy notes..." className="h-12 w-full rounded-full border border-slate-200/80 bg-[#f5f7ff] px-11 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#0f51ff] focus:bg-white focus:shadow-[0_0_0_4px_rgba(15,81,255,0.12)]" />
							</label>
						...
						<section className="space-y-5">
							<div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
								...
								<div className="flex flex-wrap gap-3">
									<button className="rounded-xl bg-[#e8edff] px-4 py-2.5 text-sm font-bold text-slate-700 shadow-[0_8px_16px_rgba(15,23,42,0.06)]">Mark all as read</button>
									<button className="rounded-xl bg-[#dce6ff] px-4 py-2.5 text-sm font-bold text-[#0f51ff] shadow-[0_8px_16px_rgba(15,23,42,0.06)]">Notification settings</button>
								</div>
							</div>

							<div className="grid gap-5 xl:grid-cols-[220px_minmax(0,1fr)_280px]">
								<aside className="rounded-[26px] border border-slate-200/80 bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.06)] sm:p-5">
									<div className="space-y-2">
										{folders.map((folder) => (
											<button key={folder.label} className={`flex w-full items-center justify-between rounded-2xl px-4 py-3 text-left text-sm font-semibold transition ${folder.active ? 'bg-[#eef3ff] text-[#0f51ff] shadow-[inset_3px_0_0_0_#0f51ff]' : 'text-slate-700 hover:bg-slate-50'}`}>
								...
								<aside className="space-y-5">
									...
									<article className="rounded-[26px] bg-[#0f51ff] p-5 text-white shadow-[0_20px_40px_rgba(15,81,255,0.3)] sm:p-6">
										...
										<button className="mt-5 inline-flex rounded-2xl bg-white px-4 py-3 text-sm font-extrabold text-[#0f51ff] shadow-[0_12px_24px_rgba(255,255,255,0.18)] transition hover:-translate-y-0.5">Review Queue</button>
									</article>
								</aside>
							</div>
						</section>
