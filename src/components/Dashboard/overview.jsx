			...
			<div className="flex h-screen w-full overflow-hidden border border-white/80 bg-white/85 shadow-[0_30px_90px_rgba(15,23,42,0.14)] backdrop-blur-xl">
				<aside className="fixed left-0 top-0 hidden h-screen shrink-0 flex-col overflow-hidden border-r border-slate-200/80 bg-[#f2f6ff]/80 px-5 py-6 xl:flex" style={{ width: '290px' }}>
					...
					<nav className="space-y-2 text-[14px] font-medium text-slate-600">
						<Link className="flex items-center gap-3 rounded-xl bg-white px-4 py-3 font-semibold text-[#0f51ff] shadow-[0_10px_24px_rgba(15,23,42,0.06)]" to="/overview"><FiHome className="h-4 w-4" /> Dashboard Overview</Link>
					...
					<header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/75 px-4 py-4 backdrop-blur-xl sm:px-6 xl:px-8">
						...
						<div className="mt-4 flex flex-col gap-4 xl:mt-0 xl:flex-row xl:items-center xl:justify-between">
							<label className="relative w-full max-w-3xl">
								...
								<input type="search" placeholder="Search resources, shifts, or members..." className="h-12 w-full rounded-full border border-slate-200/80 bg-[#f5f7ff] px-11 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#0f51ff] focus:bg-white focus:shadow-[0_0_0_4px_rgba(15,81,255,0.12)]" />
							</label>
						...
						<section id="dashboard" className="space-y-5">
							<div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
								...
								<div className="inline-flex rounded-2xl bg-[#f3f6ff] p-1 text-sm font-semibold text-slate-500 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
									<button className="rounded-xl bg-white px-4 py-2 text-[#0f51ff] shadow-[0_8px_16px_rgba(15,23,42,0.06)]">Last 24 Hours</button>
							...
							<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
								{metrics.map((metric) => {
									...
									return (
										<article key={metric.title} className="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-5 shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
										...
										{[38, 58, 74, 66, 82, 88, 72].map((height, index) => (
											<div key={weekLabels[index]} className="flex h-full flex-col items-center justify-end gap-3">
												<div className="relative flex h-full w-full items-end justify-center rounded-[18px] bg-white/40 px-2 pb-2">
													<div className="w-full rounded-2xl bg-[linear-gradient(180deg,rgba(15,81,255,0.95)_0%,rgba(15,81,255,0.42)_100%)] shadow-[0_16px_28px_rgba(15,81,255,0.18)]" style={{ height: `${height}%` }} />
							...
							<article className="rounded-[26px] border border-slate-200/80 bg-[#eef3ff] p-5 shadow-[0_12px_30px_rgba(15,23,42,0.06)] sm:p-6">
								...
								<div className="mt-5 space-y-3">
									{shiftCards.map((shift, index) => (
										<div key={shift.name} className="rounded-[18px] border border-white/80 bg-white p-4 shadow-[0_10px_24px_rgba(15,23,42,0.06)]">
								...
								<div className="mt-5 overflow-hidden rounded-[22px] bg-[#10204a] text-white shadow-[0_18px_40px_rgba(16,32,74,0.26)]">
						...
						<section id="adjustments" className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_320px]">
							...
							<div className="space-y-5">
								...
								<article className="rounded-[26px] bg-[#0f51ff] p-5 text-white shadow-[0_20px_40px_rgba(15,81,255,0.3)] sm:p-6">
									...
									<button className="mt-5 inline-flex rounded-2xl bg-white px-4 py-3 text-sm font-extrabold text-[#0f51ff] shadow-[0_12px_24px_rgba(255,255,255,0.18)] transition hover:-translate-y-0.5">Update Preferences</button>
								</article>
							</div>
						</section>
					</div>
